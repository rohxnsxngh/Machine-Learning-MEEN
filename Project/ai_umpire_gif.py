...
def all_pitches_plot(frame):
    plt.clf()
    plt.scatter(umpire_df['px'][:frame][umpire_df['pcall'] == 0], umpire_df['pz'][:frame][umpire_df['pcall'] == 0], color='blue', label='Ball')
    plt.scatter(umpire_df['px'][:frame][umpire_df['pcall'] == 1], umpire_df['pz'][:frame][umpire_df['pcall'] == 1], color='red', label='Strike')
...
def test_pitches_plot(frame):
    plt.clf()
    
    accuracy = accuracy_score(y_test[:frame], y_pred[:frame])
    
    
    plt.contourf(x0, x1, contor_values, levels=[0, 0.5, 1], colors=contour_colors, alpha=0.3)
     
    plt.scatter(X_test['px'][:frame][y_test==0], X_test['pz'][:frame][y_test==0], color = 'blue', label = 'Ball')
    plt.scatter(X_test['px'][:frame][y_test==1], X_test['pz'][:frame][y_test==1], color = 'red', label = 'Strike')
...
#%%
    ...
    # Create an animation
    animation = FuncAnimation(fig, test_pitches_plot, frames=len(umpire_df['pcall']), interval=20, repeat=False)
    
    # Save the animation as a GIF
    animation.save(f'test_pitch_animation_{HP}.gif', writer='pillow')
